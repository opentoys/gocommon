...
func TestSize(t *testing.T) {
	c := NewSM3()
	if got := c.Size(); got != SM3_Size {
		t.Errorf("Size = %d; want %d", got, SM3_Size)
	}
}

func TestBlockSize(t *testing.T) {
	c := NewSM3()
	if got := c.BlockSize(); got != SM3_BlockSize {
		t.Errorf("BlockSize = %d want %d", got, SM3_BlockSize)
	}
...
func TestKdf(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			if got := SM3_Kdf(tt.args.z, tt.args.len); !reflect.DeepEqual(got, wantBytes) {
...
func TestKdfOldCase(t *testing.T) {
	...
	result := SM3_Kdf(append(x2.Bytes(), y2.Bytes()...), 19)
