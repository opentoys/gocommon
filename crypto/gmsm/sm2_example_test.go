...
func TestExampleNewPrivateKeyFromInt(t *testing.T) {
	key := big.NewInt(0x123456)
	priv, err := gmsm.NewPrivateKeyFromInt(key)
	if err != nil {
		log.Fatalf("fail to new private key %v", err)
	}
	fmt.Printf("%x\n", priv.D.Bytes())
...
func TestExampleNewPublicKey(t *testing.T) {
	...
	pub, err := gmsm.NewPublicKey(keypoints)
	if err != nil {
		log.Fatalf("fail to new public key %v", err)
	}
	fmt.Printf("%x\n", elliptic.Marshal(gmsm.P256(), pub.X, pub.Y))
...
func TestExample_parseCipherASN1EndsWithInvalidBytes(t *testing.T) {
	// a sample method to get frist ASN1 SEQUENCE data
	getFirstASN1Sequence := func(ciphertext []byte) ([]byte, []byte, error) {
		input := gmsm.String(ciphertext)
		var inner gmsm.String
		if !input.ReadASN1(&inner, gmsm.SEQUENCE) {
	...
	input := gmsm.String(seq)
	if !input.ReadASN1(&inner, gmsm.SEQUENCE) ||
		!input.Empty() ||
		!inner.ReadASN1Integer(x1) ||
		!inner.ReadASN1Integer(y1) ||
		!inner.ReadASN1Bytes(&c3, gmsm.OCTET_STRING) ||
		!inner.ReadASN1Bytes(&c2, gmsm.OCTET_STRING) ||
		!inner.Empty() {
...
func TestExamplePrivateKey_Sign_forceSM2(t *testing.T) {
	...
	testkey, err := gmsm.NewPrivateKey(privKey)
	if err != nil {
		log.Fatalf("fail to new private key %v", err)
	}

	// force SM2 sign standard and use default UID
	sig, err := testkey.Sign(rand.Reader, toSign, gmsm.DefaultSM2SignerOpts)
	if err != nil {
...
func TestExampleVerifyASN1WithSM2(t *testing.T) {
	// real public key should be from cert or public key pem file
	keypoints, _ := hex.DecodeString("048356e642a40ebd18d29ba3532fbd9f3bbee8f027c3f6f39a5ba2f870369f9988981f5efe55d1c5cdf6c0ef2b070847a14f7fdf4272a8df09c442f3058af94ba1")
	testkey, err := gmsm.NewPublicKey(keypoints)
	if err != nil {
		log.Fatalf("fail to new public key %v", err)
	}
	...
	ok := gmsm.VerifyASN1WithSM2(testkey, nil, toSign, signature)

	fmt.Printf("%v\n", ok)
	// Output: true
}

func TestExampleEncryptASN1(t *testing.T) {
	// real public key should be from cert or public key pem file
	keypoints, _ := hex.DecodeString("048356e642a40ebd18d29ba3532fbd9f3bbee8f027c3f6f39a5ba2f870369f9988981f5efe55d1c5cdf6c0ef2b070847a14f7fdf4272a8df09c442f3058af94ba1")
	testkey, err := gmsm.NewPublicKey(keypoints)
	if err != nil {
	...
	ciphertext, err := gmsm.EncryptASN1(rng, testkey, secretMessage)
	if err != nil {
...
func TestExamplePrivateKey_Decrypt(t *testing.T) {
