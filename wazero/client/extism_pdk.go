...
func OutputMemory(mem Memory) {
	extismOutputSet(ExtismPointer(mem.Offset()), mem.Length())
}

// Output sends the `data` slice of bytes to the host output.
func Output(data []byte) {
	clength := uint64(len(data))
	m := AllocateBytes(data)

	extismOutputSet(ExtismPointer(m.Offset()), clength)
	// TODO: coordinate replacement of call to free based on SDK alignment
	// extismFree(offset)
}
...
func SetErrorString(err string) {
	...
	// TODO: coordinate replacement of call to free based on SDK alignment
	// defer mem.Free()
	extismErrorSet(ExtismPointer(mem.Offset()))
}
...
func GetConfig(key string) (string, bool) {
	...
	offset := extismConfigGet(ExtismPointer(mem.Offset()))
...
func LogMemory(level LogLevel, m Memory) {
	...
	switch level {
	case LogInfo:
		extismLogInfo(ExtismPointer(m.Offset()))
	case LogDebug:
		extismLogDebug(ExtismPointer(m.Offset()))
	case LogWarn:
		extismLogWarn(ExtismPointer(m.Offset()))
	case LogError:
		extismLogError(ExtismPointer(m.Offset()))
	case LogTrace:
		extismLogTrace(ExtismPointer(m.Offset()))
	}
}
...
func GetVar(key string) []byte {
	...
	offset := extismVarGet(ExtismPointer(mem.Offset()))
	clength := ExtismLength(offset)
	if offset == 0 || clength == 0 {
		return nil
	}
...
func SetVar(key string, value []byte) {
	...
	extismVarSet(
		ExtismPointer(keyMem.Offset()),
		ExtismPointer(valMem.Offset()),
	)
}
...
func RemoveVar(key string) {
	...
	// TODO: coordinate replacement of call to free based on SDK alignment
	// defer mem.Free()
	extismVarSet(ExtismPointer(mem.Offset()), 0)
}
...
func (r *HTTPRequest) Send() HTTPResponse {
	...
	if len(r.body) > 0 {
		...
		defer data.Free()
		dataOffset = ExtismPointer(data.Offset())
	}

	offset := ExtismHTTPRequest(
		ExtismPointer(req.Offset()),
		dataOffset,
	)
	...
	if headersOffs != 0 {
		...
		mem := NewMemory(headersOffs, length)
		defer mem.Free()
		json.Unmarshal(mem.ReadBytes(), &headers)
	}

	memory := NewMemory(offset, length)
...
func FindMemory(offset uint64) Memory {
	...
	return NewMemory(ExtismPointer(offset), length)
}
...
// ResultBytes allocates bytes and returns the offset in Extism host
func ResultBytes(d []byte) uint64 {
	mem := AllocateBytes(d)
	return mem.Offset()
}

// ResultString allocates a UTF-8 string and returns the offset in Extism host
func ResultString(s string) uint64 {
	mem := AllocateString(s)
	return mem.Offset()
}

// ResultU32 allocates a uint32 and returns the offset in Extism host
func ResultU32(d uint32) uint64 {
	mem := AllocateBytes(binary.LittleEndian.AppendUint32([]byte{}, d))
	return mem.Offset()
}

// ResultU64 allocates a uint64 and returns the offset in Extism host
